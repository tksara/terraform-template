...
resource "null_resource" "test" {
  provisioner "local-exec" {
    ...
    command = "curl -X POST https://ven01183.service-now.com/servicenowCallbackUrl.do -H 'Content-Type: application/json' -d '{\"instance_name\": \"${google_compute_instance.default.*.name[0]}\", \"gc_project\": \"${google_compute_instance.default.*.project[0]}\"}'"
    interpreter = ["cmd"]
  }
}	
